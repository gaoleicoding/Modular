package com.kymjs.app.share.view;
...
public class DragGridView extends GridView {
    ...
    private OnMoveListener moveListener; // 拖拽开始与结束监听器
    ...
    private final PostHandle mHandler = new PostHandle(this);

    private static final class PostHandle extends Handler {

        WeakReference<DragGridView> outside;

        PostHandle(DragGridView activity) {
            outside = new WeakReference<>(activity);
        }
            ...
            if (outsideView != null && outsideView.moveListener != null) {
                switch (msg.what) {
                    case HANDLE_START:
                        ...
                        break;
                    case HANDLE_CANCLE:
                        outsideView.moveListener.cancleMove();
                        break;
                    case HANDLE_FINISH:
                        outsideView.moveListener.finishMove();
                        break;
                }
            }
        }
    ...
    private boolean isTouchInItem(TouchRect moveRect, float x, float y) {
        ...
        return x < moveRect.right && x > moveRect.left && y < moveRect.bottom
                && y > moveRect.top;
    }
